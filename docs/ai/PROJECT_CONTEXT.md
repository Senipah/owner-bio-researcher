...
- `ai_research` (optional, compiled research output): dossier path and review
  ...
  HTML into that field. Schema-v8 dossier tags are resolved through the
  lifecycle-aware `config/owner-tags.json`. Owner research is closed-world:
  only active canonical ID/name pairs may appear in `proposed_tags` and reach
  `ai_research.tags`; formal candidate discovery is a separate corpus-level
  operation. These local IDs are not website IDs and are mapped only during a
  later update stage. See
  [OWNER_TAG_GOVERNANCE.md](OWNER_TAG_GOVERNANCE.md) for the binding policy.
